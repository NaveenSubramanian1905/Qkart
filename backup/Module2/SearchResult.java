...
public class SearchResult {
    ...
    public Boolean verifySizeChartExists() {
        ...
        try {
            ...
            if(sizeChart.isDisplayed()){
                String txt = sizeChart.getText();
                if(txt.equalsIgnoreCase("SIZE CHART")){
                    status = true;
                }
            }
            return status;
    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders, List<List<String>> expectedTableBody,
        ...
        try {
            ...
            for(int i = 0; i < expectedTableHeaders.size(); i++){
                ...
                String actualTableHeaderValue = actualTableHeader.getText();
                if(!actualTableHeaderValue.equals(expectedTableHeaderValue)){
                    status = false;
                }
            }
            for(int i = 0; i < expectedTableBody.size(); i++){
                List<String> expectedTableBodyRowValues = expectedTableBody.get(i);
                for(int j = 0; j < expectedTableBodyRowValues.size();j++){
                    String expectedTableBodyValues = expectedTableBodyRowValues.get(j);
                    int row = i + 1;
                    int column = j + 1;
                    String xpath = "//table/tbody/tr["+ row +"]/td["+ column +"]";
                    WebElement actualTableBody = driver.findElement(By.xpath(xpath));
                    String actualTableBodyValue = actualTableBody.getText();
                    if(!actualTableBodyValue.equals(expectedTableBodyValues)){
                        status = false;
                    }
                }
            }
    ...
    public Boolean verifyExistenceofSizeDropdown(WebDriver driver) {
        ...
        try {
            ...
            WebElement sizeDd = driver.findElement(By.xpath("//select[@id='uncontrolled-native']"));
            status = sizeDd.isDisplayed();
            return status;
        } catch (Exception e) {
            return status;
        }
